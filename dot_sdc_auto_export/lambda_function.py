    ...
    for dataset in availableDatasets:
        if 'exportWorkflow' in dataset:
            if 'CVP' in dataset['exportWorkflow']:
                cvpExportWorkflow.update(dataset['exportWorkflow']['CVP'])
    return cvpExportWorkflow
...
def lambda_handler(event, context):
    ...
    listOfPOC = {'WYDOT': retrieveCVPExportWorkflow()['WYDOT']['ListOfPOC']}
    ...
    notifyPOC(listOfPOC['WYDOT'], emailContent)
